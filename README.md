...
|------|-------------|------|---------|:--------:|
| <a name="input_custom_tags"></a> [custom\_tags](#input\_custom\_tags) | Custom tags to set on the secret managed at the destination | <pre>map(object(<br/>    {<br/>      string = string<br/>    }<br/>  ))</pre> | `{}` | no |
...
| <a name="input_secret_name_template"></a> [secret\_name\_template](#input\_secret\_name\_template) | Template describing how to generate external secret names. | `string` | `"vault_{{ .MountAccessor | lowercase }}_{{ .SecretPath | lowercase }}"` | no |
| <a name="input_secrets"></a> [secrets](#input\_secrets) | Map of vault kv to create secret sync association | <pre>map(<br/>    object({<br/>      mount       = string<br/>      secret_name = list(string)<br/>    })<br/>  )</pre> | `{}` | no |
